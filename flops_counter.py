...
from timm.models.vision_transformer import Attention as Attention_vit
from models.localvit import Attention as Attention_conv_vit
from models.tnt import Attention as Attention_tnt
from models.pvt import Attention as Attention_pvt
from models.transformer_block import Attention as Attention_t2t
from models.token_transformer import Attention as Attention_t2t_t
...
def attn_flops_counter_hook(attn_module, input, output):
    input = input[0]
    batch_size, num_token, embed_dim = input.shape
    num_heads = attn_module.num_heads
    head_dim = embed_dim // num_heads

    # q, k, v flops
    qkv_flops = attn_module.qkv.__flops__

    # q @ k flops
    if isinstance(attn_module, Attention_conv_vit):
        qk_dim = attn_module.qk_dim
        qk_head_dim = qk_dim // num_heads
        qk_flops = batch_size * num_heads * num_token * num_token * qk_head_dim
    else:
        qk_flops = batch_size * num_heads * num_token * num_token * head_dim

    # attn @ v flops
    attn_v_flops = batch_size * num_heads * num_token * num_token * head_dim

    # projection flops
    projection_flops = attn_module.proj.__flops__

    # overall_flops = qk_flops + attn_v_flops
    overall_flops = qkv_flops + qk_flops + attn_v_flops + projection_flops
    attn_module.__flops__ += int(overall_flops)


def attn_tnt_flops_counter_hook(attn_module, input, output):
    input = input[0]
    batch_size, num_token, embed_dim = input.shape
    num_heads = attn_module.num_heads
    hidden_dim = attn_module.hidden_dim

    # q, k, v flops
    qkv_flops = attn_module.qk.__flops__ + attn_module.v.__flops__

    # q @ k flops
    qk_flops = batch_size * num_heads * num_token * num_token * hidden_dim // num_heads

    # attn @ v flops
    attn_v_flops = batch_size * num_heads * num_token * num_token * embed_dim // num_heads
...
def attn_pvt_flops_counter_hook(attn_module, input, output):
    input = input[0]
    batch_size, num_token, embed_dim = input.shape
    num_heads = attn_module.num_heads

    # q, k, v flops
    qkv_flops = attn_module.q.__flops__ + attn_module.kv.__flops__
    if attn_module.sr_ratio > 1:
        qkv_flops += attn_module.sr.__flops__

    # q @ k flops
    qk_flops = batch_size * num_heads * num_token * num_token * embed_dim // num_heads
...
MODULES_MAPPING = {
    ...
    Attention_vit: attn_flops_counter_hook,
    Attention_conv_vit: attn_flops_counter_hook,
    Attention_tnt: attn_tnt_flops_counter_hook,
    Attention_pvt: attn_pvt_flops_counter_hook,
    Attention_t2t: attn_flops_counter_hook,
    Attention_t2t_t: attn_flops_counter_hook
}
